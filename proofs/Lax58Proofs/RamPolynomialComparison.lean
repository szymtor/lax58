...
namespace Lax58Proofs.RamPolynomialComparison

open Lax13.Ram
open Lax58.StructuralPresentation Lax58.StructuralCombinators
open Lax58.RamComplexity Lax58.RamPolynomialComparison
...
/--
---
conclusion: Lax58.RamPolynomialComparison.bitPolynomialTime_iff_ramPolytime
---
...
theorem bitPolynomialTime_iff_ramPolytime (f : List Nat → Nat) :
    BitPolynomialTime f ↔ Lax51.RamPolytime.RamPolytime (fun xs => [f xs]) := by
  constructor
  · intro hbit
    rcases (Lax58Proofs.BitPolynomialTime.using_iff
    ...
    have hlength : xs.length < 2 ^ wordBits.eval inputBits := by
      ...
      rw [Lax58Proofs.RamPolynomialSeparation.natList_nodes] at hnodes
      omega
    ...
    have houtput : f xs < 2 ^ wordBits.eval inputBits :=
      Lax58Proofs.RamOutputBounds.runsTo_output_fits hsourceRun (f xs)
        (by simp [natOutput])
    constructor
    · intro value hmember
    ...
    · intro w hw
      ...
      · simp [nativeTime]
        have hbits := Lax58Proofs.RamExponentialExample.length_le_bitSize xs
        omega
  ...
  · intro hnative
    ...
    apply (Lax58Proofs.BitPolynomialTime.using_iff
    ...
    · intro xs
      ...
      · change 3 * (listToRaw nat xs).nodes ≤ _
        rw [Lax58Proofs.RamPolynomialSeparation.natList_nodes, hwidthEval,
          pow_add]
        norm_num
...
end Lax58Proofs.RamPolynomialComparison
