...
namespace Lax58Proofs.RamExponentialExample
...
-- These elementary size facts use the same binary encoding as Lax51.
theorem bitSize_cons (a : Nat) (xs : List Nat) :
    bitSize (a :: xs) = a.bits.length + 1 + bitSize xs := by
  simp [bitSize, Lax51.BinaryWordEncoding.encode, encodeNat,
...
theorem natList_root (xs : List Nat) :
  ...
  have hroot := Lax58Proofs.WordArena.encodeRaw_root_last ((list nat).toRaw xs)
  have hsize := Lax58Proofs.WordArena.encodeRaw_memoryWords_proof ((list nat).toRaw xs)
  rw [hsize] at hroot
...
/--
---
conclusion: Lax58.RamPolynomialComparison.exponentialLength_bitPolynomialTime
---
...
end Lax58Proofs.RamExponentialExample
