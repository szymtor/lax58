...
namespace Lax560851Proofs.Legacy.RamPolynomialComparison
...
/--
Internal equivalence for the historical machine. It is used only through
checked transfers to and from the current public RAM predicates.
...
theorem bitPolynomialTime_iff_ramPolytime (f : List Nat → Nat) :
    BitPolynomialTime f ↔ Lax759944Proofs.Legacy.RamPolytime.RamPolytime (fun xs => [f xs]) := by
  constructor
  · intro hbit
    rcases hbit with ⟨wordBits, time, program, hfit, hprogram⟩
    ...
    obtain ⟨_, _, _, hcurrentRun⟩ := Lax759944Proofs.LegacyRamBridge.runsTo hsourceRun
    have houtput : f xs < 2 ^ wordBits.eval inputBits :=
      Lax560851Proofs.RamOutputBounds.runsTo_output_fits hcurrentRun (f xs)
        (by simp [natOutput])
    constructor
    · intro value hmember
...
end Lax560851Proofs.Legacy.RamPolynomialComparison
