...
\hypersetup{pdftitle={Two notions of polynomial time on the word-RAM},
  pdfsubject={Lax51, Lax58, input magnitude, and word capacity}}
...
\author{}
\date{Lax51 and the current Lax58 example --- 6 September 2026}

\begin{document}
\begin{center}
  {\Large\bfseries Two notions of polynomial time on the word-RAM}\par
  {\small Lax51 and the current Lax58 example\enspace\textbullet\enspace 6 September 2026}
\end{center}

The definitions differ in both their input-size measure and their required
word-width threshold. This note compares Lax51's \code{RamPolytime} with
\code{PolynomialTime} in Lax58's \code{RamComplexityExample}, not with the
unrestricted two-resource predicate \code{RamComputableWithin}.
...
\begin{aligned}
  ...
  B &= \bits(x)=\text{length of Lax51's binary encoding},\\[3pt]
...
\toprule
 & \textbf{Lax51: RamPolytime} & \textbf{Lax58: current PolynomialTime example} \\
\midrule
...
Lax51 requires the native input and exact output to fit at width $Q(B)$,
...
For a list of naturals, Lax58's three width premises are exactly
\[
...
\]
For Lax51, use the output adapter $x\mapsto[f(x)]$ to match the natural-number
output of the Lax58 example. The value $2^n$ needs $n+1$ bits, even though its
numerical value is exponential in $n$.

\textbf{Why Lax51 accommodates it.}
...
\textbf{Why the current Lax58 example excludes it.}
...
\]
This width satisfies all three Lax58 premises and is $O(\log n)$.
...
general Lax58 predicate is flexible enough to use time bound $P(B)$ and
capacity bound $2^{Q(B)}$; its capacity premise then becomes $Q(B)\le w$.
The new \code{BitPolynomialTime} definition makes this alignment while
keeping arena input. For natural-number outputs, its equivalence with Lax51 is
now proved by verified translations between arena and native length-prefixed
input tapes. Each direction chooses its own polynomial bounds; their degrees
are not required to agree. Concretely, arena-to-native conversion sends a time
bound $T(B)$ to $66T(B)+6B+81$, whereas native-to-arena conversion sends it to
$18T(B)+26$. Both send a sufficient-width bound $Q(B)$ to $Q(B)+7$. Therefore
all nonconstant time degrees are preserved, but a constant arena-time bound can
become linear because the current adapter performs linear startup work.

\small
\textit{Status.} Both the separation and the general equivalence with Lax51
are kernel-checked. See \code{Lax51.RamPolytime} and
\code{Lax58.RamPolynomialComparison} for the source definitions.
\end{document}
